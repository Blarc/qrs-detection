function Detector(record)
  ...
  % wfdb2mat -r record - generates recordm.mat
  % Detector(record) - generates record.asc
  % wrann -r record -a det <record.asc - generates record.det
  % wrann -r record -a qrs <record.asc - generates record.qrs
  % bxb -r record -a atr qrs
  % bxb -r record -a atr det

  fileName = sprintf('%s.mat', record);
  idx = QRSChenDetect(fileName);
